...
def export_db_sort_by_absolute_cursor
  ...
  for idx in 0..(roadtraffic_size/records_per_file+1)
    file_json = 'roadtraffic-v3-frozen-20120824-'+idx.to_s+'.json'
    File.open(file_json, 'w') do |file|
      file.puts RoadTraffic.where(ts: (Time.now-3*24*3600)..Time.now).asc(:ts).limit(records_per_file).offset(idx*records_per_file).to_json
    end
  end
end
