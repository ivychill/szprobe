...
def assign_tasks_v2(traffic_snap)
  ...
  traffic_snap.each do |traffic_road|
    snap.congested_roads.new :rn=>traffic_snap[:road], :href=>traffic_snap[:href]
  end
  snap.save
  #其次生成任务并
  #Spawn 进程执行路况的抓取
  ...
  for tsk in 0..(slots-1)
    ...
    arrayCrawlerTasks[tsk].snap_ts = @global_snap_ts
    arrayCrawlerTasks[tsk].carrier = "wap-traffic-crawler-worker-"+(1+tsk).to_s
  end
...
def fetch_links_v2
  ...
  begin
    ...
    doc.css("div.daolu a").each do |link|
      ...
      traffic_road = { :road => link.content, :link => wap_link}
